...
Yahoo roadmap: Y-02 ✅ → Y-03 (September) → Y-04 → Y-05.
...
| PMF-04 · Draft DNA System | Done — Spotify Wrapped-style archetype identity system. 9 archetypes (Moneyball GM → Contrarian catch-all) classified from draft picks via ADP deltas, category grades, and GM profile. DraftDNACard modal auto-shows on first draft completion; re-openable via button below Category Report. Bold prediction generated by Haiku API, cached in league state. Web Share API (navigator.share) primary sharing; clipboard fallback with "Copied!" confirmation. DraftComplete.jsx replaced by DraftRecap.jsx (kept on disk until UAT confirms). |
