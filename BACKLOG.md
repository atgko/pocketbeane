...
| Trade value index | ✅ UAT complete (sell-high/buy-low) 2026-07-26; trade-opportunity flags fixed same day, needs re-test | `/api/season/trade-value-index` — `getTradeValueIndex()`, pure POST (no live Yahoo call). Scans the user's own roster for sell-high candidates (current-season overperformance vs. baseline/ADP) and every other team's roster for buy-low targets (real pedigree, depressed current trend). Caught and fixed a real model-output bug in testing: Claude occasionally suggested "buying low" on the user's own player — added a server-side filter dropping any `buyLowTargets` entry matching the user's own roster, on top of tightening the prompt. Now also returns `tradeOpportunityFlags` (1-2 items) — a 3rd prompt job cross-referencing the sell-high/buy-low signal against deterministic per-opponent category weaknesses (`src/utils/teamStanding.js`), feeding Team Pulse's trade-opportunity section (see below). Browser UAT 2026-07-26 hit `max_tokens: 1100` truncation on real league-sized output ("Malformed JSON") — bumped to 2000, same fix `league-pulse.js` already needed in T3-5. |
| Team Pulse (Roster Health Score + League Pulse, combined 2026-07-26) | Browser UAT 2026-07-26 — insight reworked same day, needs re-test | New combined Season Hub panel, replaces the standalone League Pulse panel and the "Roster Health Score" coming-soon placeholder (which had zero prior implementation beyond that placeholder). Section 1: user's team — deterministic Contender/Playoff Bubble/Rebuilding tier + trend arrow (rank vs. top-half-of-league cutoff; no per-league playoff-spot count exists yet, defaults to top half) + current record + per-category win-rate badges (`src/utils/teamStanding.js`, unit-tested, 23 cases) + a Claude insight. **Reworked 2026-07-26** after browser UAT found the insight too thin (one generic "strongest/thinnest category" sentence): `/api/season/league-pulse` now runs a 4th prompt job, always producing a `myTeamTake` (summary/strengths/weaknesses/recommendation) grounded in real deterministic win-rate data and an approximate weeks-remaining figure (`SPORT_CONFIGS.seasonEndDate`), regardless of whether the user's team happens to land in the dominating/rebuilding lists. Section 2: full league landscape, one line per team, color-coded by tier, no fetch needed (pure client-side math). Section 3: trade-opportunity flags from the extended `trade-value-index` endpoint above. Trend arrow requires a league's *second* roster sync to have a prior snapshot to diff against (`leagueStore.js` now stashes `previousStandings` on every sync) — blank, not fabricated, on the first sync. |
